...
class StarCraft2Env(MultiAgentEnv):
    ...
    def __init__(
        ...
        reward_semi_sparse: bool, optional
            Receive 10 reward for killing an enemy unit, and recieve -5 reward
            when an ally died (default is False).
        ...
        # ✅ Stage-1/Stage-2 参数（按照文档要求）
        self.smac_stage = int(getattr(args, "smac_stage", 1))
        
        if self.smac_stage == 1:
            # Stage-1: 基础协作能力（原版SMAC设置）
            self._base_sight_range = 9.0
            self.obs_dropout_p = 0.0
            self.comm_dropout_p = 0.0
            self.vip_death_penalty = 0.0
            self.boss_kill_bonus = 0.0
        else:
            # Stage-2: 遮挡+通信不稳定+价值异质性
            self._base_sight_range = float(getattr(args, "sight_range", 6.0))
            self.obs_dropout_p = float(getattr(args, "obs_dropout_p", 0.3))
            self.comm_dropout_p = float(getattr(args, "comm_dropout_p", 0.3))
            self.vip_death_penalty = float(getattr(args, "vip_death_penalty", 200.0))
            self.boss_kill_bonus = float(getattr(args, "boss_kill_bonus", 200.0))
        
        self.vip_id = int(getattr(args, "vip_id", 0))
        self.boss_id = int(getattr(args, "boss_id", 0))
        ...
        self.reward_semi_sparse = reward_semi_sparse
        assert not (reward_sparse and reward_semi_sparse), "Cannot have both sparse and semi-sparse rewards"
    ...
    def step(self, actions):
        ...
        try:
            ...
            terminated = True
            available_actions = []
            # ✅ 按照文档要求：记录关键事件指标（VIP/Boss状态）
            vip_dead = bool(self.death_tracker_ally[self.vip_id] == 1) if (hasattr(self, 'vip_id') and self.vip_id < len(self.death_tracker_ally)) else False
            boss_dead = bool(self.death_tracker_enemy[self.boss_id] == 1) if (hasattr(self, 'boss_id') and self.boss_id < len(self.death_tracker_enemy)) else False
            ...
            for i in range(self.n_agents):
                ...
                infos[i] = {
                    ...
                    # ✅ 按照文档要求：添加Stage-2关键指标
                    "vip_dead": vip_dead,
                    "boss_dead": boss_dead,
                    "smac_stage": self.smac_stage if hasattr(self, 'smac_stage') else 1
                }
                if terminated:
                    dones[i] = True
                else:
    ...
    def reward_battle(self):
        ...
        for al_id, al_unit in self.agents.items():
            if not self.death_tracker_ally[al_id]:
                ...
                if al_unit.health == 0:
                    ...
                    # ✅ 按照文档要求：VIP死亡惩罚
                    if al_id == self.vip_id and self.vip_death_penalty > 0:
                        reward -= self.vip_death_penalty
                else:
                    # still alive
        ...
        for e_id, e_unit in self.enemies.items():
            if not self.death_tracker_enemy[e_id]:
                ...
                if e_unit.health == 0:
                    ...
                    # ✅ 按照文档要求：Boss击杀奖励
                    if e_id == self.boss_id and self.boss_kill_bonus > 0:
                        reward += self.boss_kill_bonus
                else:
        ...
        if self.reward_sparse:
            return 0
        if self.reward_semi_sparse:
            return delta_deaths # 10 * enemy_death - 5 * ally_death * (1 - reward_only_positive)
        return reward
    ...
    def unit_sight_range(self, agent_id):
        ...
        # ✅ 按照文档要求：返回self._base_sight_range（Stage-1: 9.0, Stage-2: 6.0）
        return self._base_sight_range
    ...
    def get_obs_agent(self, agent_id):
        ...
        if unit.health > 0:  # otherwise dead, return all zeros
            ...
            # ✅ 按照文档要求：enemy occlusion (mask enemy feats)
            if self.obs_dropout_p > 0:
                mask = (self._rng.rand(self.n_enemies) < self.obs_dropout_p)
                enemy_feats[mask, :] = 0
            ...
            # ✅ 按照文档要求：comm dropout (mask other allies, 保留自身)
            if self.comm_dropout_p > 0:
                for a in range(self.n_agents - 1):
                    if self._rng.rand() < self.comm_dropout_p:
                        ally_feats[a, :] = 0
